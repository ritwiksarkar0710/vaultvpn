...
class VpnViewModel @Inject constructor(
    ...
    private val serviceConnection = object : ServiceConnection {
        override fun onServiceConnected(name: ComponentName, service: IBinder) {
            ...
            viewModelScope.launch { vpnService?.vpnState?.collect { _vpnState.value = it } }
            viewModelScope.launch { vpnService?.sessionStats?.collect { _sessionStats.value = it } }
        }
        override fun onServiceDisconnected(name: ComponentName) { vpnService = null }
    }
    init { bindService(); loadServers() }
    private fun bindService() {
        try {
            val intent = Intent(context, VaultVpnService::class.java)
            context.bindService(intent, serviceConnection, Context.BIND_AUTO_CREATE)
        } catch (e: Exception) { Timber.e(e, "bindService failed") }
    }
    ...
    fun connect() {
        ...
        try {
            ...
            _vpnState.value = VpnState.Error(e.message ?: "Failed to start")
        }
    }
    fun disconnect() {
        try {
        ...
        } catch (e: Exception) { Timber.e(e, "disconnect failed") }
    ...
    fun selectServer(server: VpnServer) {
        ...
        if (_vpnState.value is VpnState.Connected) {
            disconnect()
            viewModelScope.launch { delay(800); connect() }
        }
    }
    fun setBridge(type: BridgeType) { _bridgeType.value = type }
    private fun loadServers() {
        viewModelScope.launch {
            repository.getServers().onSuccess { list ->
                _servers.value = list
                if (_selectedServer.value == null)
                    _selectedServer.value = list.firstOrNull { it.isFeatured } ?: list.firstOrNull()
